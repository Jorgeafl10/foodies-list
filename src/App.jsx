    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column",
      ...
      <div style={{ textAlign: "center", maxWidth: 340 }}>
        ...
        <p style={{ color: "#666", fontSize: 15, lineHeight: 1.6, marginBottom: 48 }}>
          La lista compartida de restaurantes de tu grupo. Descubre, guarda y visita juntos.
        </p>
        ...
        <button onClick={signInWithGoogle}
          style={{ width: "100%", padding: "16px 24px", borderRadius: 16, border: "none",
            ...
            boxShadow: "0 8px 30px rgba(0,0,0,0.4)", transition: "transform 0.15s" }}
...
function AddReelModal({ onClose, onAdd, currentUser }) {
  ...
  const handleAnalyze = async () => {
    ...
    try { const r = await analyzeReel(url); setAnalyzed(r); setStep(2); }
    catch { setError("No se pudo analizar el link. Intenta de nuevo."); }
    setLoading(false);
  };
  ...
  const handleAdd = async () => {
    ...
    setLoading(true);
    await onAdd({
      name: analyzed.name, type: analyzed.type, location: analyzed.location,
      categories: analyzed.categories?.length ? analyzed.categories : ["italian"],
  ...
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()}
      ...
      <div style={{ background: "#181818", borderRadius: "28px 28px 0 0",
        ...
        <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 900, fontSize: 22, color: "#fff", marginBottom: 6 }}>
          {step === 1 ? "➕ Agregar Restaurante" : "✨ IA Detectó"}
        </h2>
        <p style={{ color: "#555", fontSize: 13, marginBottom: 22 }}>
          {step === 1 ? "Pega el link del reel de Instagram o TikTok" : "Confirma la info antes de agregar"}
        </p>
        ...
        {step === 1 && (
          <>
            ...
            {error && <p style={{ color: "#FF3D71", fontSize: 13, marginBottom: 10 }}>{error}</p>}
        ...
        {step === 2 && analyzed && (
          <>
            <div style={{ background: "rgba(255,107,53,0.07)", border: "1px solid rgba(255,107,53,0.22)",
              borderRadius: 16, padding: 16, marginBottom: 18 }}>
              {[["🍽", "Restaurante", analyzed.name], ["🍴", "Tipo", analyzed.type], ["📍", "Ubicación", analyzed.location]].map(([icon, label, val]) => (
                <div key={label} style={{ display: "flex", gap: 10, marginBottom: 10, alignItems: "flex-start" }}>
                  <span style={{ fontSize: 16 }}>{icon}</span>
                  <div>
                    <div style={{ fontSize: 10, color: "#FF6B35", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</div>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                ...
                <div>
                  <div style={{ fontSize: 10, color: "#FF6B35", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>Categorías</div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {(analyzed.categories || []).map((cid) => {
                      const c = CATEGORIES.find((x) => x.id === cid);
                      return c ? (
                        <span key={cid} style={{ fontSize: 12, padding: "4px 10px", borderRadius: 20,
                          background: "rgba(255,107,53,0.18)", color: "#FF8C61",
                          fontWeight: 600, border: "1px solid rgba(255,107,53,0.3)" }}>
                          {c.emoji} {c.label}
                        </span>
                      ) : null;
                    })}
              ...
              {analyzed.description && (
                <p style={{ color: "#888", fontSize: 12, fontStyle: "italic", marginTop: 12 }}>"{analyzed.description}"</p>
              )}
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => { setStep(1); setAnalyzed(null); }}
                style={{ flex: 1, padding: "14px", borderRadius: 14,
                  border: "1px solid rgba(255,255,255,0.1)", background: "transparent",
                  color: "#777", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>← Volver</button>
              <button onClick={handleAdd} disabled={loading}
                style={{ flex: 2, padding: "14px", borderRadius: 14, border: "none",
                  background: "linear-gradient(135deg, #FF6B35, #FF3D71)", color: "#fff",
                  fontSize: 14, fontWeight: 800, fontFamily: "'Syne', sans-serif", cursor: "pointer",
      ...
      <div style={{ padding: "12px 24px 60px", display: "flex", flexDirection: "column", gap: 12 }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 20px" }}>
            ...
            <p style={{ color: "#555", fontSize: 14 }}>Agrega un reel de un lugar de {cat?.label?.toLowerCase()}</p>
          </div>
        ) : filtered.map((r, i) => (
...
export default function App() {
  ...
  useEffect(() => {
    ...
    sbFetch("/auth/v1/user", {}, session.access_token)
      .then(setUser)
      .catch(() => {});
  }, [session, user]);
  ...
  const loadRestaurants = useCallback(async () => {
    ...
    setSyncing(true);
    try {
      const data = await getRestaurants(session.access_token);
      setRestaurants(data || []);
    } catch {}
    setSyncing(false);
  }, [session]);
            ...
            {filteredList.length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px 20px" }}>
                ...
                <p style={{ color: "#555", fontSize: 14, lineHeight: 1.6 }}>
                  {tab === "wishlist" ? "Agrega tu primer reel con el botón +" : "Marca restaurantes como visitados para verlos aquí"}
                </p>
              </div>
